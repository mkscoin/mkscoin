...
const char * const mkscoin_CONF_FILENAME = "Mkscoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "mkscoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\mkscoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\mkscoin
    // Mac: ~/Library/Application Support/mkscoin
    // Unix: ~/.mkscoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mkscoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Mkscoin";
#else
    // Unix
    return pathRet / ".Mkscoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", mkscoin_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure mkscoin Core copyright is not removed by accident
    if (copyright_devs.find("mkscoin Core") == std::string::npos) {
        std::string strYear = strPrefix;
        strYear.replace(strYear.find("2011"), sizeof("2011")-1, "2009");
        strCopyrightHolders += "\n" + strYear + "The mkscoin Core developers";
    }
