...
`test/functional/test_framework/test_framework.py:mkscoinTestFramework.main`.
...
def main():
    ...
    enable_mkscoind = config["components"].getboolean("ENABLE_mkscoinD")

    if not enable_mkscoind:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, runs_ci):
    ...
    # Warn if mkscoind is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "Mkscoind"]) is not None:
            print("%sWARNING!%s There is already a Mkscoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
...
class RPCCoverage():
    ...
    commands per `Mkscoin-cli help` (`rpc_interface.txt`).
