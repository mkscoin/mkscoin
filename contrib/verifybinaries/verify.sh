#!/usr/bin/env bash
# Copyright (c) 2016 The mkscoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   mkscoincore.org and mkscoin.org and compares them.
...
WORKINGDIR="/tmp/mkscoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://mkscoincore.org"
HOST2="https://mkscoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="mkscoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'mkscoin-',
...
if [ $? -ne 0 ]; then
   echo "mkscoin.org failed to provide signature file, but mkscoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "mkscoin.org and mkscoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the mkscoin Core binary release signing key installed?"
   fi
